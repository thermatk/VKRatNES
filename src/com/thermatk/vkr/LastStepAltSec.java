...
public class LastStepAltSec {
    ...
    private String makeStat(String[] csvIn, int index) {
        ...
        if(data.length > 1) {
            ...
            Kurtosis kurtObject = new Kurtosis();
            double kurtosis = kurtObject.evaluate(data);
            res = mean + cvsSplitBy + stdErr + cvsSplitBy + stdDev + cvsSplitBy + kurtosis + cvsSplitBy + pValue;
        } else {
            res = "not enough data";
        }
        return res;
    ...
    private double[] myCCFFile(double[] x, double[] y, int maxDelay, String filepath) {
        ...
        String[] ccfArray = new String[ccfdata.size()];
